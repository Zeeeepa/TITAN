 ...
 *  - New threshold (5 per-target in 1h) replaces the old (2 per-target in 24h)
...
describe('killSwitch — non-exempt paths still trigger', () => {
    it('fires kill when a production file hits the new 5× threshold', async () => {
        // 4 events → still armed
        for (let i = 0; i < 4; i++) {
            recordFixOscillation('file:/opt/TITAN/src/agent/agent.ts');
        }
        expect(getState().status).toBe('armed');
    ...
    it('does NOT fire at the OLD 2× threshold on non-exempt paths', () => {
        recordFixOscillation('file:/opt/TITAN/src/skills/foo.ts');
        recordFixOscillation('file:/opt/TITAN/src/skills/foo.ts');
        expect(getState().status).toBe('armed');
    });
});
