...
describe('systemPromptParts — size budget', () => {
    ...
    it('full mode without dynamicContext stays under ~3.5KB', () => {
        const p = assembleSystemPrompt({
            modelId: 'ollama/gemma4:31b-cloud',
            persona: 'default',
            mode: 'full',
        });
        expect(p.length, `full mode is ${p.length} chars, ceiling 3500`).toBeLessThan(3500);
    });

    it('minimal mode without dynamicContext stays under ~2.5KB', () => {
        const p = assembleSystemPrompt({
            modelId: 'ollama/gemma4:31b-cloud',
            persona: 'default',
            mode: 'minimal',
        });
        expect(p.length, `minimal mode is ${p.length} chars, ceiling 2500`).toBeLessThan(2500);
    });
    ...
    it('full mode is dramatically smaller than the legacy ~25KB template', () => {
        ...
        // creeps back above 5KB, someone is re-introducing the MUST/NEVER walls.
        const p = assembleSystemPrompt({
            modelId: 'ollama/gemma4:31b-cloud',
            persona: 'default',
            mode: 'full',
        });
        expect(p.length, `full is ${p.length}, legacy was ~25000 — regression budget 5000`).toBeLessThan(5000);
    });
});
