...
export function HelpPanel({ open, onClose }: HelpPanelProps) {
  ...
  return (
    <div className="fixed inset-y-0 right-0 w-96 max-w-full bg-[var(--bg-primary)] border-l border-[var(--border)] shadow-2xl z-50 flex flex-col animate-slide-in-right">
      {/* Header */}
      <div className="flex items-center justify-between p-4 border-b border-[var(--border)]">
        <div className="flex items-center gap-2">
          <HelpCircle size={20} className="text-[var(--accent)]" />
          <h2 className="text-lg font-semibold text-[var(--text)]">Help</h2>
        </div>
        <button
          onClick={onClose}
          className="p-1 rounded-md text-[var(--text-secondary)] hover:text-[var(--text)] hover:bg-[var(--bg-tertiary)] transition-colors"
        >
          <X size={18} />
        </button>
      </div>
      ...
      <div className="p-3 border-b border-[var(--border)]">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2 text-sm rounded-lg border border-[var(--border)] bg-[var(--bg-secondary)] text-[var(--text)] placeholder:text-[var(--text-muted)] focus:outline-none focus:ring-1 focus:ring-[var(--accent)]"
          />
        </div>
      </div>
              ...
              {filteredFaq.map((faq, i) => (
                ...
                >
                  <button
                    ...
                    className="w-full px-3 py-2.5 text-left text-sm font-medium text-[var(--text)] hover:bg-[var(--bg-tertiary)] transition-colors flex items-start gap-2"
                  >
                    <Zap size={14} className="text-[var(--accent)] mt-0.5 flex-shrink-0" />
                    <span>{faq.question}</span>
                  </button>
                  {expandedFaq === i && (
                    <div className="px-3 pb-3 text-sm text-[var(--text-secondary)] leading-relaxed border-t border-[var(--border)] pt-2 ml-6">
                      {faq.answer}
                    </div>
                  )}
                </div>
          ...
          <div className="space-y-2">
            {filteredGlossary.map(([term, definition]) => (
              <div key={term} className="rounded-lg border border-[var(--border)] bg-[var(--bg-secondary)] p-3">
                <div className="text-sm font-semibold text-[var(--accent)] mb-1">{term}</div>
                <div className="text-sm text-[var(--text-secondary)] leading-relaxed">{definition}</div>
              </div>
            ))}
