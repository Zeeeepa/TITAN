...
export function ActivityCards({ events }: { events: AgentEvent[] }) {
  ...
  return (
    <div ref={scrollRef} className="h-full overflow-y-auto p-3 space-y-2">
      ...
      {/* Waterfall timeline */}
      {cards.length > 1 && (
        <div className="bg-[#18181b] rounded-lg border border-[#3f3f46] p-2 mb-2">
          ...
          <div className="flex justify-between mt-1">
            <span className="text-[10px] text-[#52525b]">{cards.length} tools</span>
      ...
      {cards.map((card) => {
        ...
        return (
          <div
            key={card.id}
            className="bg-[#18181b] rounded-lg border border-[#3f3f46] overflow-hidden transition-all duration-300"
            style={{ borderLeftWidth: 3, borderLeftColor: getBorderColor(card) }}
          >
            {/* Header */}
            <button
              onClick={() => setExpandedId(isExpanded ? null : card.id)}
              className="w-full flex items-center gap-2 px-3 py-2 hover:bg-[#27272a] transition-colors duration-200"
            >
              {getIcon(card)}
              {getToolIcon(card.name)}
              <span className="text-sm font-medium text-[#fafafa] flex-1 text-left">{card.name}</span>
              {card.durationMs !== undefined && (
                <span className="text-xs bg-[#27272a] rounded-full px-2 py-0.5 text-[#a1a1aa]">
                  {card.durationMs}ms
                </span>
              )}
              <ChevronDown
            ...
            >
              <div className="px-3 pb-3 space-y-2">
                {card.args && Object.keys(card.args).length > 0 && (
                  <div>
                    <span className="text-[10px] uppercase tracking-wider text-[#52525b]">Input</span>
                    <pre className="mt-1 text-xs font-mono text-[#a1a1aa] bg-[#09090b] rounded-md p-2 overflow-x-auto max-h-20 overflow-y-auto">
                ...
                {card.result && (
                  <div>
                    <span className="text-[10px] uppercase tracking-wider text-[#52525b]">Output</span>
                    <pre className="mt-1 text-xs font-mono text-[#a1a1aa] bg-[#09090b] rounded-md p-2 overflow-x-auto max-h-20 overflow-y-auto whitespace-pre-wrap">
                      {card.result}
                    </pre>
                  </div>
                )}
      ...
      {/* Thinking indicator */}
      {isThinking && (
        <div className="flex items-center gap-2 px-3 py-2 bg-[#18181b] rounded-lg border border-[#3f3f46]" style={{ borderLeftWidth: 3, borderLeftColor: '#f59e0b' }}>
